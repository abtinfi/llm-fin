...
mkdir -p logs/scaled .pipeline_state

echo "=== SCALED PIPELINE START $(date -Is) ==="
git rev-parse --short HEAD 2>/dev/null || echo "(no git rev)"

python src/check_data.py --strict --json results/data_provenance.json
CHECK_RC=$?
...
bash run_lane_a.sh > logs/scaled/lane_a.log 2>&1 &
PID_A=$!
bash run_lane_b.sh > logs/scaled/lane_b.log 2>&1 &
PID_B=$!
echo "lane A pid=$PID_A -> logs/scaled/lane_a.log"
echo "lane B pid=$PID_B -> logs/scaled/lane_b.log"

wait "$PID_A"; RC_A=$?
wait "$PID_B"; RC_B=$?
echo "lane A exit=$RC_A   lane B exit=$RC_B"

bash run_join.sh > logs/scaled/join.log 2>&1
RC_J=$?
echo "join exit=$RC_J"

echo "=== SCALED PIPELINE END $(date -Is) ==="
if [ "$RC_A" -eq 0 ] && [ "$RC_B" -eq 0 ] && [ "$RC_J" -eq 0 ]; then
  touch .pipeline_state/SCALED_PIPELINE_COMPLETE
  echo "ALL STAGES COMPLETE"
else
  echo "one or more lanes/join reported a non-zero exit -- check logs/scaled/*.log and re-launch to resume (checkpointed stages will be skipped)"
fi
